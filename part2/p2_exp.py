...
def run_trial(output_handle, bw=100, loss=0, delay_c2_ms=5, udp_off_mean=None, iteration=0, buffer_size=420):
    ...
    s1_pid_raw = s1.cmdPrint(f"bash -c 'python3 {server_py} {s1.IP()} {SERVER_PORT1} > /tmp/s1_{bw}_server.out 2>&1 & echo $!'").strip()
    s2_pid_raw = s2.cmdPrint(f"bash -c 'python3 {server_py} {s2.IP()} {SERVER_PORT2} > /tmp/s2_{bw}_server.out 2>&1 & echo $!'").strip()
...
def run_trial_with_udp(output_handle, bw=100, loss=0, delay_c2_ms=5, udp_off_mean=1.0, iteration=0, buffer_size=420):
    ...
    server_py = 'p2_server.py'
    s1_pid_raw = s1.cmd(f"bash -c 'python3 {server_py} {s1.IP()} {SERVER_PORT1} > /tmp/s1_{udp_off_mean}_server.out 2>&1 & echo $!'").strip()
    s2_pid_raw = s2.cmd(f"bash -c 'python3 {server_py} {s2.IP()} {SERVER_PORT2} > /tmp/s2_{udp_off_mean}_server.out 2>&1 & echo $!'").strip()
    s1_pid = s1_pid_raw.split()[0] if s1_pid_raw else None
    s2_pid = s2_pid_raw.split()[0] if s2_pid_raw else None
