...
class CustomTopo(Topo):
    def build(self, loss, delay, jitter):
        ...
        # Add links
        self.addLink(h1, s1, loss=loss, delay=f'{delay}ms', jitter=f'{jitter}ms', bw = 1.4)
        ...
        for DELAY in delay_list:
            for JITTER in jitter_list:
                for i in range(0, NUM_ITERATIONS):
                    ...
                    server_logs_folder = './server_logs'
                    client_logs_folder = './client_logs'
                    h1.cmd(f"python3 p1_server.py {SERVER_IP} {SERVER_PORT} {SWS} > {server_logs_folder}/server_loss{LOSS}_delay{DELAY}_jitter{JITTER}_iter{i}.log 2>&1 &")
                    time.sleep(1)
                    result = h2.cmd(f"python3 p1_client.py {SERVER_IP} {SERVER_PORT} > {client_logs_folder}/client_loss{LOSS}_delay{DELAY}_jitter{JITTER}_iter{i}.log 2>&1")
                    end_time = time.time()
